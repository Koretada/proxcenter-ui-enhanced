...
function NodesTableWidget({ data, loading }) {
  ...
  const headerBg = isDark ? 'rgba(255,255,255,0.05)' : 'rgba(255,255,255,0.05)'
    ...
    >
      <TableContainer sx={{ height: '100%', overflow: 'auto' }}>
        <Table size='small' stickyHeader>
          <TableHead>
            <TableRow>
              <TableCell sx={{ fontWeight: 800, bgcolor: headerBg, fontSize: 12, py: 1, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>{t('dashboard.widgets.nodes')}</TableCell>
              <TableCell sx={{ fontWeight: 800, bgcolor: headerBg, fontSize: 12, py: 1, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>{t('inventory.clusters')}</TableCell>
              <TableCell align='center' sx={{ fontWeight: 800, bgcolor: headerBg, fontSize: 12, py: 1, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>{t('common.status')}</TableCell>
              <TableCell sx={{ fontWeight: 800, bgcolor: headerBg, minWidth: 100, fontSize: 12, py: 1, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>{t('monitoring.cpu')}</TableCell>
              <TableCell sx={{ fontWeight: 800, bgcolor: headerBg, minWidth: 100, fontSize: 12, py: 1, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>{t('monitoring.memory')}</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {[...nodes].sort((a, b) => (b.memPct || 0) - (a.memPct || 0)).map((node, idx) => (
              ...
              >
                <TableCell sx={{ py: 0.75, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    <Box sx={{ position: 'relative', display: 'inline-flex', width: 18, height: 18, flexShrink: 0 }}>
                      ...
                      <Box sx={{ position: 'absolute', bottom: -1, right: -1, width: 8, height: 8, borderRadius: '50%', bgcolor: node.status === 'online' ? '#4caf50' : '#f44336', border: '1.5px solid', borderColor: isDark ? 'rgba(255,255,255,0.03)' : '#1e1e2d' }} />
                    </Box>
                ...
                <TableCell align='center' sx={{ py: 0.75, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                  <Chip
                    size='small'
